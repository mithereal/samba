defmodule SambaWeb.TopicPostsLive do
  ...
  def handle_event("change-per-page", %{"per_page" => per_page}, socket) do
    {:noreply, push_navigate(socket, to: ~p"/topic/#{socket.assigns.topic_id}?page=1&per_page=#{per_page}")}
  end
  ...
  def handle_event("quote-post", %{"id" => post_id}, socket) do
    ...
    if post do
      ...
      form = to_form(%{"message" => new_msg, "subject" => "Re: #{socket.assigns.topic.topic_title}"})
  ...
  def handle_event("save-reply", %{"reply" => %{"message" => message, "subject" => subject}}, socket) do
    ...
    result = Samba.Repo.transaction(fn ->
    ...
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-2 py-8 text-gray-100">
      ...
      <div class="mb-6">
        ...
        <.breadcrumb>
          ...
          <:item icon="hero-folder-open" link={@topic && @topic.forum_id && ~p"/forum/#{@topic.forum_id}" || "/"}>
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between mb-8 px-2 gap-4">
        <div class="flex space-x-2">
          <%= if @page > 1 do %>
            <.link patch={~p"/topic/#{@topic_id}?page=#{@page - 1}&per_page=#{@per_page}"} class="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded-md text-sm font-medium border border-gray-700 transition-colors">
              &larr; Previous
            </.link>
          <% end %>
      ...
      <div class="bg-gray-900/90 border border-gray-700 rounded-lg p-6 shadow-xl">
        ...
        <.form for={@form} phx-submit="save-reply" as={:reply} class="space-y-4">
          <div>
            ...
            <input type="text" name="reply[subject]" value={@form[:subject].value || "Re: #{@topic && @topic.topic_title}"} class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-indigo-500" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-gray-400 uppercase mb-1">Message</label>
            <textarea name="reply[message]" rows="5" required class="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-gray-100 focus:outline-none focus:border-indigo-500"><%= @form[:message].value %></textarea>
          </div>

          <div>
            <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded text-sm transition-colors shadow">
              Submit Reply
            </button>
          </div>
        </.form>
  ...
  defp post_card(assigns) do
    ~H"""
    <div id={"post-#{@post.post_id}"} class="grid grid-cols-1 md:grid-cols-12 bg-gray-900/80 border border-gray-700 rounded-lg shadow-xl overflow-hidden backdrop-blur-md">
      <!-- Author Sidebar -->
        ...
        <div>
          <div class="flex justify-between items-center border-b border-gray-800 pb-2 mb-4 text-xs text-gray-400">
            ...
            <div class="flex items-center space-x-3">
              ...
              <button phx-click="quote-post" phx-value-id={@post.post_id} class="bg-gray-800 hover:bg-gray-700 text-indigo-300 px-2 py-1 rounded text-xs border border-gray-700 transition-colors">
                Quote
              </button>
            </div>
          </div>
