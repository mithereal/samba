defmodule SambaWeb.AuthController do
  use SambaWeb, :controller
  use AshAuthentication.Phoenix.Controller
  alias Samba.Accounts.UserImpersonation
  ...
  defp force_sign_in(conn, current_user_id, perpose \\ "Impersonation") do
    ...
    |> Samba.Accounts.User.force_sign_in!(authorize?: false)
  end
  ...
  def login(conn, %{"email" => email, "password" => password} = _params) do
    case Samba.Accounts.User.sign_in_with_password(%{email: email, password: password},
           authorize?: false
         ) do
      {:ok, user} ->
        conn
